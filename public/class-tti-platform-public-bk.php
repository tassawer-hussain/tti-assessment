...
class TTI_Platform_Public_Main_Class {
    ...
    */
    ...
    public function listener_shortcode() {
        ...
        if (is_user_logged_in()) {
            ...
            if($asseListRetakeAsseStatus == 'true') {
                $getLink = get_transient('assessmentListenerRetakeAsseLink'.$current_user->ID);
    ...
    */
    public function initiate_group_leader_email_process($ass_id, $report_id, $api_key, $api_service, $current_user_id, $user_email_data, $assessment_id, $report_view_status) {
    ...
    * @param integer $assessment_id contains assessment id
    */
    public function group_leader_email($report_id, $api_key, $api_service, $current_user_id, $user_email_data, $assessment_id, $report_view_status) {
            ...
            // }
                ...
                if (is_wp_error($response)) {
                    ...
                    if(count($mails_to_sent) > 0) {
                        ...
                        $this->send_mail_to_group_leaders($mails_to_sent, $downloadPath, $current_user_id, $user_email_data, $assessment_id);
    ...
    public function get_groupleader_mails_by_asses_id ($user_id) {
        ...
        //     }
        // }
        $asseListGroupLeaders = get_transient('assessmentListenerGroupLeaders'.$current_user->ID);
    ...
    */
    ...
    protected function send_mail_to_group_leaders($mails_to_sent, $downloadPath, $current_user, $user_email_data, $assessment_id) {
        global $current_user;
        $asseListGroupLeaders = get_transient('assessmentListenerGroupLeaders'.$current_user->ID);
        ...
        if (is_user_logged_in()) {
            ...
            if ($results) {
                ...
                if ($gen_char_feedback == 'feedback') {
                                ...
                                if ($value['type'] == $type) {
                                    foreach ($value['statements'] as $key => $value) {
                                        ...
                                        foreach ($value['stmts'] as $key => $value) {
                                            ...
                                            $o .= '<input type="checkbox" name="' . esc_attr($ident) . '[]" id="isSelected-' . esc_attr($randstr) . '" value="" text="' . stripslashes($text) . '" ident="' . esc_attr($ident) . '" ' . esc_attr($checked) . ' /> <label for="isSelected-' . esc_attr($randstr) . '"><span>' . stripslashes($text) . '</span></label>';
                                        }
                                        $o .= '</ul>';
                                    }
                                    $getBool = true;
                                    ...
                                    foreach ($assessmenrArr[0]['statements'] as $key => $value) {
                                        ...
                                        if ($format == 'para' && $style == 'indent') {
                                            foreach ($value->stmts as $index => $checklist) {
                                                $randstr = $this->generateRandomString();
                                                $o .= '<input type="checkbox" name="' . esc_attr($ident) . '[]" id="isSelected-' . esc_attr($randstr) . '" value="" text="' . stripslashes($checklist) . '" ident="' . esc_attr($ident) . '" /> <label for="isSelected-' . esc_attr($randstr) . '"><span>' . stripslashes($checklist) . '</span></label>';
                                            }
                                            $inSelect = true;
                                        }
                                        $o .= '</ul>';
                    ...
                    } else if($type == 'EQTABLES2') {  /* EQTABLES2 Select */
                            ...
                            if (!$getBool) { 
                                ...
                                foreach ($gen_char_parArr as $key => $value) { 
                                        ...
                                        foreach ($assessmenrArr[0]['pages'][0]->bullets as $index => $checklist) {
                                                ...
                                                $o .= '<input type="checkbox" name="' . esc_attr($ident) . '[]" id="isSelected-' . esc_attr($randstr) . '" value="" text="' . esc_attr($checklist) . '" ident="' . esc_attr($ident) . '" /> <label for="isSelected-' . esc_attr($randstr) . '"><span>' . esc_html($checklist) . '</span></label>';
                    ...
                    } else if($type == 'EQGENCHAR'){ /* EQGENCHAR Select */
                        if (!empty($gen_char_parArr)) {
                            $getBool = false;
                            $inSelect = false;
                            $o .= '<div class="selectFeedbackData" id="' . $type . '">';
        ...
        if ( 
            ...
            if (is_user_logged_in()) {
                ...
                if (is_wp_error($response)) {
                    ...
                    $path = $firstName.'-'.$lastName.'-'.$slug.'-'.$matches[0].'.pdf';
                    file_put_contents($path, $data);
                    $content = file_get_contents($path);
                    header('Content-Type: application/pdf');
                    header('Content-Length: ' . strlen($content));
                    header('Content-Disposition: attachment; filename="'.$firstName.'-'.$lastName.'-'.$slug.'-'.$matches[0].'.pdf"');
